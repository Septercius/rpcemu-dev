...
int get_virtual_key_index(size_t k)
{
    ...
    {
        if (key_map[k].virtual_key[i] == 0xFFFF)
        {
            break;
        }
...
void keyboard_handle_modifier_keys(unsigned long mask)
{
    size_t k;
...
{
    if (!strcmp(layoutName, "British"))
    {
        keyboardType = keyboardLayoutBritish;
    }
    else if (!strcasecmp(layoutName, "French"))
    {
        keyboardType = keyboardLayoutFrench;
    }
    else
    {
        keyboardType = keyboardLayoutUndefined;
    }
