 ...
 */
static int nametouidgid(const char *user, uid_t *uid, gid_t *gid)
{
...
static int tun_alloc(void)
{
    ...
    assert(config.network_type == NetworkType_EthernetBridging || config.network_type == NetworkType_IPTunnelling);
    ...
    else if (config.network_type == NetworkType_EthernetBridging)
    {
        ...
        {
            error("Error interface index for %s: %s", ifr.ifr_name, strerror(errno));
            return -1;
        }
        ...
        {
            error("Error adding %s to bridge: %s", ifr.ifr_name, strerror(errno));
            return -1;
        }
    }
    ...
    else
    {
        ...
        {
            error("Error getting %s hardware address: %s", ifr.ifr_name, strerror(errno));
            return -1;
        }
...
uint32_t network_plt_tx(uint32_t errbuf, uint32_t mbufs, uint32_t dest, uint32_t src, uint32_t frametype)
{
    ...
    *buf++ = (uint8_t) frametype;
    ...
    do
    {
        ret = write(tunfd, buffer, packet_length);
 ...
 * @return errbuf on error, else zero
 */
uint32_t network_plt_rx(uint32_t errbuf, uint32_t mbuf, uint32_t rxhdr, uint32_t *data_avail)
{
...
void network_plt_setirqstatus(uint32_t address)
{
    ...
    if (irqstatus != 0)
    {
        ...
        {
            error("Error disabling SIGIO for network device: %s", strerror(errno));
            return;
        }
    ...
    if (irqstatus != 0)
    {
        ...
        if (fcntl(tunfd, F_SETFL, flags | O_ASYNC) == -1)
        {
            error("Error enabling SIGIO for network device: %s", strerror(errno));
            return;
        }
    }
}
...
int network_plt_init(void)
{
    ...
    { /* Running as root */
        ...
        {
            fatal("Could not find username '%s' on the system, or error", user);
        }
