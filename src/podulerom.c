 ...
 */
static void podulerom_write8(podule *p, PoduleIoType io_type, uint32_t addr, uint8_t val)
{
    NOT_USED(p);
 ...
 */
static uint8_t podulerom_read8(podule *p, PoduleIoType io_type, uint32_t addr)
{
    NOT_USED(p);
 ...
 */
void podulerom_reset(void)
{
    self = addpodule(NULL, NULL, podulerom_write8, NULL, NULL, podulerom_read8, NULL, NULL);
...
void initpodulerom(void)
{
    ...
    for (i = 0; i < file; i++)
    {
        ...
        if (len < 0)
        {
            fatal("initpodulerom: Error reading size of podule ROM file '%s': %s", romfns[i], strerror(errno));
        }
        if (len > 4096 * 1024)
        {
        ...
        {
            fatal("initpodulerom: Failed to read file '%s': %s", romfns[i], strerror(errno));
        }
        fclose(f);
 ...
 */
void podulerom_mouse_wheel_change(int dy)
{
    if (self == NULL || !enable)
    {
