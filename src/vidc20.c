 ...
 */
static uint32_t *video_image_scanline(int row)
{
 ...
 * @param yh
 */
static void video_update(int yl, int yh)
{
    rpcemu_video_update(thr.bitmap, current_sizex, current_sizey, yl, yh, thr.doublesize, thr.host_xsize, thr.host_ysize);
}
 ...
 */
void vidc_get_doublesize(int *double_x, int *double_y)
{
 ...
 */
static uint32_t makecol(uint32_t r, uint32_t g, uint32_t b)
{
    return (0xff << 24) | (r << 16) | (g << 8) | b;
}

static void vidc_palette_update(void)
{
    int i;
    ...
    for (i = 0; i < 256; i++)
    {
        ...
        thr.palette[i] = makecol(vidc.palette[i] & 0xff, (vidc.palette[i] >> 8) & 0xff, (vidc.palette[i] >> 16) & 0xff);
    }
    for (i = 0; i < 3; i++)
    {
        thr.cursor_palette[i] = makecol(vidc.cursor_palette[i] & 0xff, (vidc.cursor_palette[i] >> 8) & 0xff, (vidc.cursor_palette[i] >> 16) & 0xff);
    }
    thr.border_colour = makecol(vidc.border_colour & 0xff, (vidc.border_colour >> 8) & 0xff, (vidc.border_colour >> 16) & 0xff);
}
    ...
    switch (val >> 28)
    {
        ...
        case 9: /* Vertical Registers */
            switch (val >> 24)
            {
                ...
                default:
                    UNIMPLEMENTED("VIDC Horiz/Vert", "Unknown register 0x%08x 0x%02x", val, val >> 24);
            }
            break;
        ...
        default:
            UNIMPLEMENTED("VIDC register", "Unknown register 0x%08x 0x%x", val, val >> 28);
            break;
    }
}
