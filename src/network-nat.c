...
static void write16(FILE *f, uint16_t x)
{
    (void) fwrite(&x, sizeof(uint16_t), 1, f);
}

static void write32(FILE *f, uint32_t x)
{
 ...
 * @param data_len
 */
static void write_packet(FILE *f, const void *data, size_t data_len)
{
    struct timeval tv;
...
/**
 */
static void network_nat_init_mac_address(void)
{
...
/**
 */
static void network_nat_open(void)
{
    struct in_addr host;
    struct in_addr mask;
    ...
    if (nat.slirp == NULL)
    {
        ...
        nat.slirp = slirp_init(restricted, net_addr, mask, host, vhostname, "", bootfile, dhcp, dns, NULL);
 ...
 */
uint32_t network_nat_tx(uint32_t errbuf, uint32_t mbufs, uint32_t dest, uint32_t src, uint32_t frametype)
{
 ...
 * @return errbuf on error, else zero
 */
uint32_t network_nat_rx(uint32_t errbuf, uint32_t mbuf, uint32_t rxhdr, uint32_t *data_avail)
{
 ...
 * @param address
 */
void network_nat_setirqstatus(uint32_t address)
{
 ...
 */
void network_nat_forward_add(PortForwardRule rule)
{
    struct in_addr bind = { 0 };
    int retval;

    // Inform SLIRP of the rule added
    retval = slirp_add_hostfwd(nat.slirp, rule.type == PORT_FORWARD_UDP ? 1 : 0, bind, rule.host_port, nat.forward_addr, rule.emu_port);
    if (retval != 0)
    {
        error("Failed to add NAT Network port forwarding rule, %s emu_port %u host_port %u, %d %d %s", rule.type == PORT_FORWARD_UDP ? "UDP" : "TCP", rule.emu_port, rule.host_port, retval, errno, strerror(errno));
    }
}
 ...
 * @param rule Details of NAT rule
 */
void network_nat_forward_remove(PortForwardRule rule)
{
    struct in_addr bind = { 0 };

    // Inform SLIRP of the rule removal
    slirp_remove_hostfwd(nat.slirp, rule.type == PORT_FORWARD_UDP ? 1 : 0, bind, rule.host_port);
}
 ...
 */
void network_nat_forward_edit(PortForwardRule old_rule, PortForwardRule new_rule)
{
