...
static void adf_readsector(int drive, int sector, int track, int side, int density)
{
    ...
    if (!adf[drive].f || (side && !adf[drive].dblside) || (density != adf[drive].density) || (track != adf[drive].track) || (sector_nr >= adf[drive].maxsector) || (adf_state.sector > adf[drive].sectors))
    {
...
static void adf_readaddress(int drive, int track, int side, int density)
{
    ...
    if (!adf[drive].f || (side && !adf[drive].dblside) || (density != adf[drive].density) || (track != adf[drive].track))
    {
        adf_state.notfound = 500;
        return;
    }
...
static disc_funcs adf_disc_funcs = { .seek = adf_seek, .readsector = adf_readsector, .writesector = adf_writesector, .readaddress = adf_readaddress, .poll = adf_poll, .format = adf_format, .stop = adf_stop, .close = adf_close };
