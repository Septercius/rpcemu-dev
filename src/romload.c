 ...
 */
static void romload_patch(void)
{
    const rom_patch_t *p;
    size_t i;
    ...
    for (i = 0, p = rom_patch; i < sizeof(rom_patch) / sizeof(rom_patch[0]); i++, p++)
    {
        ...
        if (rom[addr >> 2] == data[0] && rom[(addr + 4) >> 2] == data[1] && rom[(addr + 8) >> 2] == data[2] && rom[(addr + 12) >> 2] == data[3])
        {
            // Patch the data
...
void loadroms(void)
{
    ...
    for (c = 0; c < number_of_files; c++)
    {
        ...
        if (f == NULL)
        {
            fatal("Can't open ROM file '%s': %s", filepath, strerror(errno));
        }
        ...
        if (len < 0)
        {
            fatal("Error reading size of ROM file '%s': %s", romfilenames[c], strerror(errno));
        }
        ...
        {
            fatal("Error reading from ROM file '%s': %s", romfilenames[c], strerror(errno));
        }
     ...
     *       8MB (Current maximum)
     */
    if (pos != (2 * 1024 * 1024) && pos != (4 * 1024 * 1024) && pos != (6 * 1024 * 1024) && pos != (8 * 1024 * 1024))
    {
    ...
    for (c = 0; c < pos; c += 4)
    {
        ...
        rom[c >> 2] = (temp >> 24) | ((temp >> 8) & 0x0000ff00) | ((temp << 8) & 0x00ff0000) | (temp << 24);
    }
#endif
