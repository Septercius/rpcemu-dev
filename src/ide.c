...
#define IDE_DRIVE_IS_CDROM(ide) (config.cdromenabled && (ide.drive == 1))
...
static inline void ide_irq_raise(void)
{
    iomd.irqb.status |= IOMD_IRQB_IDE;
    updateirqs();
}

static inline void ide_irq_lower(void)
{
    iomd.irqb.status &= ~IOMD_IRQB_IDE;
    updateirqs();
}
 ...
 */
static void ide_atapi_identify(void)
{
    memset(ide.buffer, 0, 512);
 ...
 */
static uint32_t ide_atapi_mode_sense(uint32_t pos)
{
    ...
    buf[pos++] = 0;
    buf[pos++] = 2; /* Number of audio levels - on and off only */
    buf[pos++] = 0;
 ...
 */
static off64_t ide_get_sector(void)
{
    ...
    else
    {
        ...
        return ((((off64_t) ide.cylinder * heads) + ide.head) * sectors) + (ide.sector - 1) + skip;
    }
}
 ...
 */
static void ide_image_set_spt_hpc_skip512(FILE *fh, int d)
{
    int log2_sec_size;
    ...
    if ((ide.spt[d] == 0 || ide.spt[d] == EOF) || (ide.hpc[d] == 0 || ide.hpc[d] == EOF))
    {
 ...
 */
static void loadhd(int d, const char *filename)
{
    char pathname[512];
    ...
    if (ide.hdfile[d] == NULL)
    {
        ...
        if (ide.hdfile[d] == NULL)
        {
            ...
            if (errno == ENOENT)
            {
                ...
                if (ide.hdfile[d] == NULL)
                {
                    fatal("Cannot create file '%s': %s", pathname, strerror(errno));
                }
            }
            else
            {
                /* Failed for another reason */
                fatal("Cannot open file '%s': %s", pathname, strerror(errno));
            }
        }
    }
    ...
    rpclog("IDE: Loaded file '%s' as IDE disc %d, size %" PRId64 " MB (%" PRId64 ")%s\n", filename, d, (int64_t) filesize / 1024 / 1024, (int64_t) filesize, ide.skip512[d] ? ", BUG 512b Skip enabled" : "");
}
...
static void atapicommand(void)
{
    ...
    switch (idebufferb[0])
    {
        ...
        case GPCMD_READ_SUBCHANNEL:
            ...
            idebufferb[pos++] = 0;
            idebufferb[pos++] = 0; /*Subchannel length*/
