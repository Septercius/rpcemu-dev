...
#include "hostfs-multi-macosx.h"
#elif defined(RPCEMU_PLATFORM_WIN32)
#include "hostfs-multi-win.h"
#endif
...
#else
static void dbug_hostfs(const char *format, ...)
{
    va_list ap;
 ...
 */
static int hostfs_find_drive_from_name(char *discName)
{
 ...
 * @return          The index of the drive, or -1 if not found.
 */
static int hostfs_find_drive_from_path(const char *ro_path)
{
    char driveName[13];
    int i = 0;
 ...
 */
static void hostfs_ensure_buffer_size(size_t buffer_size_needed)
{
 ...
 */
static void get_string(ARMul_State *state, ARMword address, char *buf, size_t bufsize)
{
    assert(state);
    assert(buf);
 ...
 */
static ARMword put_string(ARMul_State *state, ARMword address, const char *str)
{
    ARMword len = 0;
 ...
 */
static void hostfs_object_set_timestamp(const char *host_path, ARMword load, ARMword exec)
{
...
static void riscos_path_to_host(const char *path, char *host_path, bool leaf)
{
    assert(path);
    assert(host_path);
    ...
    if (!leaf)
    {
        // The input should be something in the format
        // ':<drive>.$.<folder>.<file>'.
        assert((char) *path == '.');
        path++;
 ...
 */
static void name_host_to_riscos(const char *object_name, size_t len, char *riscos_name)
{
    assert(object_name);
    assert(riscos_name);
 ...
 */
static void path_construct(const char *old_path, const char *ro_path, char *new_path, size_t len, ARMword load, ARMword exec)
{
    const char *ro_leaf;
    char *new_suffix;
 ...
 */
static void hostfs_read_object_info(const char *host_pathname, char *ro_leaf, risc_os_object_info *object_info)
{
    ARMword file_type;
    ...
    if (comma)
    {
        ...
        if (dash)
        {
            ...
            if ((dash - comma - 1) >= 1 && (dash - comma - 1) <= 8 && strlen(dash + 1) >= 1 && strlen(dash + 1) <= 8)
            {
                ...
                if (!whitespace)
                {
                    ...
                    {
                        /* Replace timestamp information with load-exec
                         * addresses */
    ...
    if (ro_leaf)
    {
        ...
        if (truncate_name)
        {
            /* If a filetype or load-exec was found, we only want the part from
               after the slash to before the comma */
 ...
 * @param object_info   Return object info (filled-in)
 */
static void hostfs_path_scan(const char *host_dir_path, const char *object, char *host_name, risc_os_object_info *object_info)
{
    DIR *d;
    struct dirent *entry;
    size_t c;
 ...
 */
static enum FILECORE_ERROR hostfs_path_process(const char *ro_path, char *host_pathname, risc_os_object_info *object_info)
{
            ...
            case '.':
                if (component_name[0] != '\0')
                {
                    ...
                    *component = '\0'; /* add terminator */
                    ...
                    hostfs_path_scan(host_pathname, component_name, host_name, object_info);
                    if (object_info->type == OBJECT_TYPE_NOT_FOUND)
                    {
                    ...
                    /* Append Host's name for this component to the working Host
                     * path */
                    path_append(host_pathname, host_name);
 ...
 */
static unsigned hostfs_open_allocate_index(void)
{
    unsigned i;
...
static void hostfs_open(ARMul_State *state)
{
    ...
    if (object_info.type == OBJECT_TYPE_NOT_FOUND)
    {
        /* FIXME RISC OS uses this to create files - not therefore an error if
         * not found */
    ...
    /* TODO Handle the case that a file exists to be replaced, (and the filetype
       is not data - the recommended default for new files) */
...
static void hostfs_getbytes(ARMul_State *state)
{
...
static void hostfs_putbytes(ARMul_State *state)
{
    FILE *f = open_file[state->Reg[1]];
    ARMword ptr = state->Reg[2];
    ARMword i;
...
static void hostfs_args_3_write_file_extent(ARMul_State *state)
{
    FILE *f;
    int fd;
...
static void hostfs_args_7_ensure_file_size(ARMul_State *state)
{
    FILE *f;
...
static void hostfs_args_8_write_zeros(ARMul_State *state)
{
...
static void hostfs_args_9_read_file_datestamp(ARMul_State *state)
{
    assert(state);
...
static void hostfs_args(ARMul_State *state)
{
    assert(state);
...
static void hostfs_close(ARMul_State *state)
{
    FILE *f;
    ARMword load, exec;
 ...
 *                  (Save) or not (Create)
 */
static void hostfs_write_file(ARMul_State *state, bool with_data)
{
    const unsigned BUFSIZE = MINIMUM_BUFFER_SIZE;
    ...
    switch (object_info.type)
    {
        case OBJECT_TYPE_NOT_FOUND:
            ...
            path_construct(host_pathname, ro_path, new_pathname, sizeof(new_pathname), state->Reg[2], state->Reg[3]);
            break;
        ...
        case OBJECT_TYPE_FILE:
            ...
            {
                fprintf(stderr, "hostfs_file_7_create_file(): could not rename \'%s\' to \'%s\': %s %d\n", host_pathname, new_pathname, strerror(errno), errno);
                return;
            }
            break;
...
static void hostfs_file_0_save_file(ARMul_State *state)
{
    assert(state);
...
static void hostfs_file_1_write_cat_info(ARMul_State *state)
{
    char ro_path[PATH_MAX], host_pathname[PATH_MAX];
    risc_os_object_info object_info;
...
static void hostfs_file_5_read_cat_info(ARMul_State *state)
{
    char ro_path[PATH_MAX], host_pathname[PATH_MAX];
    risc_os_object_info object_info;
...
static void hostfs_file_6_delete(ARMul_State *state)
{
    char ro_path[PATH_MAX], host_pathname[PATH_MAX];
    risc_os_object_info object_info;
    ...
    switch (object_info.type)
    {
        ...
        case OBJECT_TYPE_DIRECTORY:
            if (rmdir(host_pathname))
            {
                ...
                {
                    case EEXIST:
                    case ENOTEMPTY: /* POSIX permits either error for directory
                                       not empty */
...
static void hostfs_file_7_create_file(ARMul_State *state)
{
    assert(state);
...
static void hostfs_file_8_create_dir(ARMul_State *state)
{
    char ro_path[PATH_MAX];
    ...
    if (mkdir(host_pathname, 0777))
    {
        ...
        {
            case EEXIST:
                /* The object exists (not necessarily as a directory) - does it
                 * matter that it could be a file? TODO */
                return; /* Return with no error */

            case ENOSPC: /* No space for the directory (either physical or
                            quota) */
...
static void hostfs_file_255_load_file(ARMul_State *state)
{
    const unsigned BUFSIZE = MINIMUM_BUFFER_SIZE;
    char ro_path[PATH_MAX], host_pathname[PATH_MAX];
    risc_os_object_info object_info;
...
static void hostfs_file(ARMul_State *state)
{
    assert(state);
...
static void hostfs_func_0_chdir(ARMul_State *state)
{
    char ro_path[PATH_MAX];
...
static void hostfs_func_7_set_options(ARMul_State *state)
{
    int index;
...
static void hostfs_func_8_rename(ARMul_State *state)
{
    ...
    /* TODO When we support multiple virtual disks, check that rename would be 'simple' */
    ...
    {
        /* The new named object does exist - check it is similar to the old name */
    ...
    path_construct(host_pathname2, ro_path2, new_pathname, sizeof(new_pathname), object_info1.load, object_info1.exec);
 ...
 */
static int hostfs_directory_entry_compare(const void *e1, const void *e2)
{
 ...
 */
static void hostfs_cache_dir(const char *directory_name)
{
    ...
    while ((entry = readdir(d)) != NULL)
    {
        ...
        hostfs_read_object_info(entry_path, ro_leaf, &cache_entries[entry_ptr].object_info);
    ...
    qsort(cache_entries, entry_ptr, sizeof(cache_directory_entry), hostfs_directory_entry_compare);
 ...
 *                       the timestamp.
 */
static void hostfs_read_dir(ARMul_State *state, bool with_info, bool with_timestamp)
{
...

    {
        ...
        while ((count < num_objects_to_read) && (offset < cache_entries_count))
        {
            ...
            if (with_info)
            {
                if (with_timestamp)
                {
                    /* Space required for info with timestamp:
                       6 words of info, a 5-byte timestamp, and the string,
                       rounded up */
            ...
            /* See whether there is space left in the buffer to return this
             * entry */
            ...
            if (with_info)
            {
                ...
                if (with_timestamp)
                {
                    ...
                    {
                        ARMul_StoreWordS(state, ptr + 24, (cache_entries[offset].object_info.load << 24) | (cache_entries[offset].object_info.exec >> 8));
                        ARMul_StoreByte(state, ptr + 28, cache_entries[offset].object_info.exec & 0xff);
                    }
                    else
                    {
...
static void hostfs_func_11_read_name_and_boot_option(ARMul_State *state)
{
    assert(state);
    ...
    // The format for the result is '<name length byte><drive name><boot
    // option>'.
    char buffer[90];
...
static void hostfs_func_14_read_dir(ARMul_State *state)
{
    assert(state);
...
static void hostfs_func_15_read_dir_info(ARMul_State *state)
{
    assert(state);
...
static void hostfs_func_19_read_dir_info_timestamp(ARMul_State *state)
{
    assert(state);
 ...
 * @param state Emulator state
 */
static void hostfs_func_23_canonicalise_disc_name(ARMul_State *state)
{
    char disc_name[1024];
    int index;
    HostFSDrive *drive = NULL;
 ...
 * @param state Emulator state
 */
static void hostfs_func_24_resolve_wildcard(ARMul_State *state)
{
 ...
 * @param state Emulator state
 */
static void hostfs_func_27_read_boot_option(ARMul_State *state)
{
    assert(state);
 ...
 * @param state Emulator state
 */
static void hostfs_func_28_write_boot_option(ARMul_State *state)
{
    assert(state);
 ...
 * @param state Emulator state
 */
static void hostfs_func_30_read_free_space32(ARMul_State *state)
{
    disk_info d;
    int index;
 ...
 * @param state Emulator state
 */
static void hostfs_func_34_directory_changed(ARMul_State *state)
{
    char ro_path[PATH_MAX];
 ...
 * @param state Emulator state
 */
static void hostfs_func_35_read_free_space64(ARMul_State *state)
{
    disk_info d;
    int index;
...
static void hostfs_func(ARMul_State *state)
{
    assert(state);
...
static void hostfs_gbpb(ARMul_State *state)
{
    assert(state);
 ...
 * @param state Emulator state
 */
static void hostfs_register(ARMul_State *state)
{
    assert(state);
 ...
 * @param state Emulator state
 */
void hostfs_swi(ARMul_State *state)
{
    assert(state);
    ...
    if (hostfs_state == HOSTFS_STATE_REGISTERED)
    {
        switch (state->Reg[9])
        {
            case 0:
                ...
                break;
            case 1:
                hostfs_getbytes(state);
                break;
            case 2:
                hostfs_putbytes(state);
                break;
            case 3:
                hostfs_args(state);
                break;
    ...
    else if (hostfs_state == HOSTFS_STATE_UNREGISTERED)
    {
        /* Log attempt to use HostFS without registration and ignore further operations */
        rpclog("HostFS: Attempt to use HostFS without registration - ignoring\n");
        hostfs_state = HOSTFS_STATE_IGNORE;
    }
    else if (hostfs_state == HOSTFS_STATE_IGNORE)
    {
 ...
 */
void hostfs_swi_initialise(ARMul_State *state)
{
    // HostFS_Initialise
    //
    // On entry:
 ...
 * @param state Emulator state
 */
void hostfs_swi_drives(ARMul_State *state)
{
    // HostFS_Drives
    //
    // On exit:
 ...
 * @param state Emulator state
 */
void hostfs_swi_getdrivename(ARMul_State *state)
{
    // HostFS_GetDriveName
    //
    // On entry:
    //    R0 = drive number.
    //    R1 = 0 to calculate buffer length, or address of buffer to fill with
    //    drive name. R2 = buffer length.
    //
    // On exit:
    //    R0 = preserved.
    //    R1 = preserved.
 ...
 * @param state Emulator state
 */
void hostfs_swi_generatecommand(ARMul_State *state)
{
 ...
 * @param state Emulator state
 */
void hostfs_swi_freeop(ARMul_State *state)
{
    assert(state);
    ...
    switch (state->Reg[0])
    {
        ...
        case HOSTFS_FREE_OPERATION_FREE64:
            ...
            // This returns the root folder which can be passed to OS_FSControl
            // 49 or 55.
            dbug_hostfs("\tr2 = 0x%08x (pointer to buffer).\n", state->Reg[2]);
            dbug_hostfs("\tr3 = 0x%08x (pointer to device name/identifier).\n", state->Reg[3]);
 ...
 * @param state Emulator state
 */
void hostfs_swi_validatedrive(ARMul_State *state)
{
 ...
 * @param state Emulator state
 */
void multi_hostfs_swi_dispatch(int swinum, ARMul_State *state)
{
    assert(state);
    ...
    switch (swinum)
    {
        ...
        case ARCEM_SWI_MULTI_HOSTFS_INITIALISE:
            hostfs_swi_initialise(state);
            break;

        case ARCEM_SWI_MULTI_HOSTFS_DRIVES:
            hostfs_swi_drives(state);
            break;

        case ARCEM_SWI_MULTI_HOSTFS_GET_DRIVE_NAME:
            hostfs_swi_getdrivename(state);
            break;

        case ARCEM_SWI_MULTI_HOSTFS_GENERATE_COMMAND:
            hostfs_swi_generatecommand(state);
            break;

        case ARCEM_SWI_MULTI_HOSTFS_FREEOP:
            hostfs_swi_freeop(state);
            break;

        case ARCEM_SWI_MULTI_HOSTFS_VALIDATE_DRIVE:
