 ...
 */
static int hostfs_disc_name_valid(const char *disc_name)
{
...
static void riscos_path_to_host(const char *path, char *host_path)
{
    assert(path);
    assert(host_path);
...
static void hostfs_write_file(ARMul_State *state, bool with_data)
{
    ...
    switch (object_info.type)
    {
        ...
        case OBJECT_TYPE_FILE:
            ...
            {
                fprintf(stderr,
                    "hostfs_file_7_create_file(): could not rename \'%s\'"
                    " to \'%s\': %s %d\n",
                    host_pathname,
                    new_pathname,
                    strerror(errno),
