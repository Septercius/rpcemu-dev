...
static const Format formats[] = { { "ADFS D/E 800KB", "adf", 2, 80, 5, 1024, 0, 2 }, { "ADFS F 1600KB", "adf", 2, 80, 10, 1024, 0, 0 }, { "ADFS L 640KB", "adl", 2, 80, 16, 256, 0, 2 }, { "DOS 360KB", "img", 2, 40, 9, 512, 1, 2 }, { "DOS 720KB", "img", 2, 80, 9, 512, 1, 2 }, { "DOS 1440KB", "img", 2, 80, 18, 512, 1, 0 } };

int fdccallback = 0;
int motoron = 0;

static inline void fdc_irq_raise(void)
{
    iomd.irqb.status |= IOMD_IRQB_FLOPPY;
    updateirqs();
}

static inline void fdc_irq_lower(void)
{
    iomd.irqb.status &= ~IOMD_IRQB_FLOPPY;
    updateirqs();
}

static inline void fdc_dma_raise(void)
{
    iomd.fiq.status |= IOMD_FIQ_FLOPPY_DMA_REQUEST;
    updateirqs();
}

static inline void fdc_dma_lower(void)
{
    iomd.fiq.status &= ~IOMD_FIQ_FLOPPY_DMA_REQUEST;
    updateirqs();
}
 ...
 */
void fdc_image_load(const char *fn, int drive)
{
    FILE *f;
    const char *extension;
    long len;
    ...
    {
        adf_load(drive, fn, format->sectors, format->sectorsize, format->sides, format->tracks == 40, format->density ? 1 : 2, format->sectorskew);
    }
}

void fdc_image_save(const char *fn, int drive)
{
    (void) fn;
    if (drive_funcs[drive])
    {
...
void fdc_write(uint32_t addr, uint32_t val)
{
    switch (addr)
    {
        ...
        case 0x3f5: /* Data (FIFO) - Command */
            if (fdc.params)
            {
                ...
                if (fdc.curparam == fdc.params)
                {
                    ...
                    switch (fdc.command)
                    {
                        ...
                        default:
                            UNIMPLEMENTED("FDC command", "Unknown command 0x%02x", fdc.command);
                    }
                }
                return;
            }
            ...
            switch (fdc.command)
            {
                ...
                default:
                    UNIMPLEMENTED("FDC command 2", "Unknown command 0x%02x", fdc.command);
...
uint8_t fdc_read(uint32_t addr)
{
    switch (addr)
    {
...
uint8_t fdc_dma_read(uint32_t addr)
{
    fdc_dma_lower();
    fdc.data_ready = 0;
    if (addr == 0x302a000)
...
void fdc_dma_write(uint32_t addr, uint8_t val)
{
    fdc_dma_lower();
    if (addr == 0x302a000)
    {
