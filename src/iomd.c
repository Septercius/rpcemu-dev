...
{
    if ((iomd.irqa.status & iomd.irqa.mask) || (iomd.irqb.status & iomd.irqb.mask) || (iomd.irqd.status & iomd.irqd.mask) || (iomd.irqdma.status & iomd.irqdma.mask))
    {
        arm.event |= 1;
    }
    else
 ...
 */
void iomd_write(uint32_t addr, uint32_t val)
{
    uint32_t reg;
    ...
    switch (reg)
    {
        ...
        case IOMD_0x00C_IOLINES: /* General Purpose I/O lines (ARM7500/FE) */
            if (iomd_type == IOMDType_ARM7500 || iomd_type == IOMDType_ARM7500FE)
            {
                ...
                if ((val & 0xfe) != 0)
                {
                    UNIMPLEMENTED("IOMD ARM7500 IOLINES", "Write to enable unknown IO device 0x%x", val);
                }
            }
            return;
        ...
        case IOMD_0x0CC_DMAEXT: /* DMA external control (IOMD) */
            ...
            UNIMPLEMENTED("IOMD Register write", "Write to register 0xcc in ARM7500/FE mode val 0x%x", val);
            return;
        ...
        case IOMD_0x0E0_ATODICR: /* A to D interrupt control (ARM7500/FE) */
            ...
            if (val != 0)
            {
                UNIMPLEMENTED("IOMD ATODICR write", "Unsupported A to D control write 0x%x", val);
            }
            return;
        ...
        case IOMD_0x1D8_VIDSTART:             /* Video DMA Start */
            ...
            if (!((iomd.vidstart >= 0x02000000 && iomd.vidstart < 0x03000000) || (iomd.vidstart >= 0x10000000 && iomd.vidstart < 0x14000000)))
            {
        ...
        default:
            UNIMPLEMENTED("IOMD Register write", "Unknown register 0x%x val 0x%x", reg, val);
            return;
    }
}
 ...
 */
uint32_t iomd_read(uint32_t addr)
{
    uint32_t reg;
    ...
    switch (reg)
    {
        ...
        case IOMD_0x0CC_DMAEXT: /* DMA external control     (IOMD) */
            ...
            UNIMPLEMENTED("IOMD Register read", "Read from register 0xcc in ARM7500/FE mode");
            return 0;
        ...
        default:
            UNIMPLEMENTED("IOMD Register read", "Unknown register 0x%x", reg);
    }
    return 0;
}
 ...
 */
uint32_t iomd_mouse_buttons_read(void)
{
    uint32_t temp = 0;
 ...
 */
void iomd_flyback(int flyback_new)
{
    flyback = flyback_new;
