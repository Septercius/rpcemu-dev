...
HostFSDialog::HostFSDialog(Emulator &emulator, Config *config_copy, Model *model_copy, QWidget *parent)
  : QDialog(parent)
  , emulator(emulator)
  ...
  , model_copy(model_copy)
{
    ...
    for (int i = 0; i < HOSTFS_DRIVE_MAX; i++)
    {
        connect(driveCheckBox[i], &QCheckBox::checkStateChanged, this, &HostFSDialog::checkBoxChanged);
 ...
 * A key was pressed.
 */
void HostFSDialog::keyPressEvent(QKeyEvent *event)
{
...
void HostFSDialog::done(int r)
{
    ...
    if (r == QDialog::Accepted)
    {
        for (int i = 0; i < HOSTFS_DRIVE_MAX; i++)
        {
            ...
            if (str.length() == 0)
            {
                snprintf(message, 1024, "You must enter a name for drive %d.", config_copy->hostfs_drive[i].id);
            ...
            if (str.length() == 0)
            {
                snprintf(message, 1024, "You must enter a path for drive %d.", config_copy->hostfs_drive[i].id);
                QMessageBox::information(this, "HostFS", message);
            ...
            {
                snprintf(message, 1024, "The path for drive %d does not exist or is inaccessible %s.", config_copy->hostfs_drive[i].id, resolvedPath);
                QMessageBox::information(this, "HostFS", message);
            ...
            {
                snprintf(message, 1024, "The path for drive %d is not a directory.", config_copy->hostfs_drive[i].id);
                QMessageBox::information(this, "HostFS", message);
 ...
 */
void HostFSDialog::dialogAccepted()
{
    QString str;
    QByteArray ba;
    ...
    if (rpcemu_hostfs_is_reset_required(&new_config))
    {
        int dialogResult = MainWindow::reset_question(parentWidget());
 ...
 */
void HostFSDialog::dialogRejected()
{
    applyConfig();
}
 ...
 */
void HostFSDialog::applyShading(int drive)
{
    bool enabled;
 ...
 */
void HostFSDialog::checkBoxChanged(int state)
{
    Q_UNUSED(state);
 ...
 */
void HostFSDialog::pathButtonPressed()
{
    int index = sender()->objectName().toInt();
