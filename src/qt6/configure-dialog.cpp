...
void ConfigureDialog::keyPressEvent(QKeyEvent *evt)
{
...
void ConfigureDialog::slider_moved(int value)
{
 ...
 */
void ConfigureDialog::dialog_accepted()
{
    ...
    if (mem_4->isChecked())
    {
        new_config.mem_size = 4;
    }
    if (mem_8->isChecked())
    {
        new_config.mem_size = 8;
    }
    if (mem_16->isChecked())
    {
        new_config.mem_size = 16;
    }
    if (mem_32->isChecked())
    {
        new_config.mem_size = 32;
    }
    if (mem_64->isChecked())
    {
        new_config.mem_size = 64;
    }
    if (mem_128->isChecked())
    {
        new_config.mem_size = 128;
    }
    if (mem_256->isChecked())
    {
 ...
 */
void ConfigureDialog::dialog_rejected()
{
    // Set the values in the dialog back to the current settings
    applyConfig();
}
 ...
 */
void ConfigureDialog::applyConfig()
{
    // Hardware Model
    ...
    {
        case 4:
            mem_4->setChecked(true);
            break;
        case 8:
            mem_8->setChecked(true);
            break;
        case 16:
            mem_16->setChecked(true);
            break;
        case 32:
            mem_32->setChecked(true);
            break;
        case 64:
            mem_64->setChecked(true);
            break;
        case 128:
            mem_128->setChecked(true);
            break;
        case 256:
            mem_256->setChecked(true);
            break;
        default:
            fatal("configuredialog.cpp: unhandled memsize %u", config_copy->mem_size);
    }
