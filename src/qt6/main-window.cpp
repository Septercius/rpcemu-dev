...
void MainDisplay::mouseMoveEvent(QMouseEvent *event)
{
    ...
    if ((!pconfig_copy->mousehackon && mouse_captured) || full_screen)
    {
        ...
        int dx = event->position().x() - middle.x();
        int dy = event->position().y() - middle.y();
    ...
    else if (pconfig_copy->mousehackon)
    {
        ...
        emit this->emulator.mouse_move_signal(event->position().x(), event->position().y());
    }
}
...
void MainDisplay::resizeEvent(QResizeEvent *)
{
    if (terminating)
    {
        return;
...
void MainDisplay::get_host_size(int &xsize, int &ysize) const
{
    xsize = this->host_xsize;
    ysize = this->host_ysize;
}

void MainDisplay::set_full_screen(bool full_screen)
{
...
void MainDisplay::update_image(const QImage &img, int yl, int yh, int double_size)
{
    ...
    else
    {
        ...
        const int bytes = (int) img.bytesPerLine() * lines;
 ...
 * - Widget size
 */
void MainDisplay::calculate_scaling()
{
    if (double_size & VIDC_DOUBLE_X)
    {
 ...
 */
int MainDisplay::get_double_size()
{
    return double_size;
}
 ...
 */
bool MainDisplay::save_screenshot(QString filename)
{
 ...
 */
int MainWindow::reset_question(QWidget *parent)
{
    QMessageBox msgBox(parent);

    msgBox.setWindowTitle("RPCEmu");
    msgBox.setText("Are you sure you want to reset RPCEmu?");
 ...
 */
void MainWindow::application_state_changed(Qt::ApplicationState state)
{
    if (terminating)
    {
        return;
 ...
 */
void MainWindow::release_held_keys()
{
...
void MainWindow::closeEvent(QCloseEvent *event)
{
    ...
    QMessageBox msgBox(QMessageBox::Question, "RPCEmu", "Are you sure you want to exit?", QMessageBox::Cancel, this);
 ...
 */
void MainWindow::native_keypress_event(unsigned scan_code, unsigned modifiers)
{
    if (terminating)
    {
        return;
 ...
 * @param modifiers Modifier key state
 */
void MainWindow::native_keyrelease_event(unsigned scan_code, unsigned modifiers)
{
    if (terminating)
    {
        return;
...
{
    QString fileName = QFileDialog::getSaveFileName(this, tr("Save Screenshot"), "screenshot.png", tr("PNG (*.png)"));
...
void MainWindow::load_disc(int drive)
{
    QString fileName = QFileDialog::getOpenFileName(this, tr("Open Disc Image"), "", tr("All disc images (*.adf *.adl *.hfe *.img);;ADFS D/E/F Disc Image (*.adf);;ADFS L Disc Image (*.adl);;DOS Disc Image (*.img);;HFE Disc Image (*.hfe)"));
...
void MainWindow::menu_loaddisc0()
{
    load_disc(0);
}

void MainWindow::menu_loaddisc1()
{
    load_disc(1);
}
 ...
 * @return filter list
 */
static QString getCreateBlankDiscFilters()
{
    QString filterList = "";
    ...
    {
        if (i != 0)
        { // We do not add a separator before first entry
            filterList += ";;";
        }
 ...
 */
static const discTypeFileMap *filter_to_disc_type_file(QString filterName)
{
    for (size_t i = 0; i < sizeof(discTypeFileMaps) / sizeof(discTypeFileMap); i++)
    {
 ...
 */
void MainWindow::create_disc(int drive)
{
    const QString filterList = getCreateBlankDiscFilters();

    QString selectedFilter;
    QString fileName = QFileDialog::getSaveFileName(this, tr("Create Blank Disc Image"), "", filterList, &selectedFilter);
    ...
    bool permissionsSuccess = QFile::setPermissions(fileName, QFileDevice::ReadOwner | QFileDevice::WriteOwner | QFileDevice::ReadUser | QFileDevice::WriteUser);
 ...
 */
void MainWindow::menu_create_disc0()
{
    create_disc(0);
}
 ...
 */
void MainWindow::menu_create_disc1()
{
    create_disc(1);
}

void MainWindow::menu_configure()
{
...
#ifdef FEATURE_NETWORKING
void MainWindow::menu_networking()
{
 ...
 */
void MainWindow::menu_fullscreen()
{
    if (!full_screen)
    {
...
void MainWindow::menu_cdrom_disabled()
{
...
void MainWindow::menu_cdrom_empty()
{
    if (!config_copy.cdromenabled)
    {
        int ret = MainWindow::reset_question(this);
...
void MainWindow::menu_cdrom_iso()
{
    QString fileName = QFileDialog::getOpenFileName(this, tr("Open ISO Image"), "", tr("ISO CD-ROM Image (*.iso);;All Files (*.*)"));
 ...
 */
void MainWindow::menu_aboutToShow()
{
    release_held_keys();
    this->menu_open = true;
}
 ...
 */
void MainWindow::menu_aboutToHide()
{
 ...
 */
void MainWindow::add_menu_show_hide_handlers()
{
...
void MainWindow::main_display_update(VideoUpdate video_update)
{
    if (video_update.host_xsize != display->width() || video_update.host_ysize != display->height())
    {
        if (!full_screen)
        {
    ...
    // Copy image data
    display->update_image(video_update.image, video_update.yl, video_update.yh, video_update.double_size);
}
 ...
 */
void MainWindow::move_host_mouse(MouseMoveUpdate mouse_update)
{
    QPoint pos;
    ...
    if (double_size == VIDC_DOUBLE_X || double_size == VIDC_DOUBLE_BOTH)
    {
        pos.setX(mouse_update.x * 2);
    }

    if (double_size == VIDC_DOUBLE_Y || double_size == VIDC_DOUBLE_BOTH)
    {
...
#ifdef FEATURE_NETWORKING
void MainWindow::send_nat_rule_to_gui(PortForwardRule rule)
{
 ...
 * MIPS and Average.
 */
void MainWindow::mips_timer_timeout()
{
...
#if 1
    // Update window title
    window_title = QString("RPCEmu - MIPS: %1 AVG: %2%3").arg(mips, 0, 'f', 1).arg(average, 0, 'f', 1).arg(capture_text);
    ...
    window_title = QString("RPCEmu - MIPS: %1 AVG: %2, ITimer: %3, VTimer: %4%5").arg(mips, 0, 'f', 1).arg(average, 0, 'f', 1).arg(icount).arg(vcount).arg(capture_text);
#endif
 ...
 * @param error error string
 */
void MainWindow::fatal(QString error)
{
 ...
 */
void MainWindow::cdrom_menu_selection_update(const QAction *cdrom_action)
{
...
bool MainWindow::nativeEvent(const QByteArray &eventType, void *message, qintptr *result)
{
    ...
    // not handle it
    if ((msg->message == WM_SYSKEYDOWN || msg->message == WM_SYSKEYUP) && (msg->wParam == VK_MENU || msg->wParam == VK_F10))
    {
