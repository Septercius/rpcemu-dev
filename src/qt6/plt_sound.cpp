...
//	QAudioDeviceInfo info(QMediaDevices::defaultAudioOutput());
...
AudioOut::changeSampleRate(uint32_t samplerate)
{
	...
	format.setChannelConfig(QAudioFormat::ChannelConfigStereo);         // Stereo
	format.setSampleFormat(QAudioFormat::Int16);          // 16 bit sound

	audio_output = new QAudioSink(outputDevice, format);
...
plt_sound_buffer_free(void)
{
	...
	if(audio_out->audio_output) {
		return (int32_t) audio_out->audio_output->bytesFree();
	} else {
