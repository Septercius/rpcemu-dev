...
static struct
{
    ...
    struct
    { /**< Mouse bounding box, defined with OS_Word 21, 1, values are in OS units */
        int16_t left;
        int16_t right;
        int16_t bottom;
        int16_t top;
...
static inline void keyboard_irq_tx_raise(void)
{
    iomd.irqb.status |= IOMD_IRQB_KEYBOARD_TX;
    updateirqs();
}

static inline void mouse_irq_tx_raise(void)
{
    iomd.irqd.status |= IOMD_IRQD_MOUSE_TX;
    updateirqs();
}

static inline void mouse_irq_tx_lower(void)
{
    iomd.irqd.status &= ~IOMD_IRQD_MOUSE_TX;
    updateirqs();
}
...
static void ps2_queue(PS2Queue *q, uint8_t b)
{
...
static int calculateparity(uint8_t v)
{
    int c, d = 0;
 ...
 */
void keyboard_data_write(uint8_t v)
{
    switch (v)
    {
        case KBD_CMD_RESET:
 ...
 * @param v Value to write
 */
void keyboard_control_write(uint8_t v)
{
    if (v && !kbd.enable)
    {
        kbd.reset = 1;
...
static void keyboardsend(uint8_t v)
{
    kbd.data = v;
...
/* Cannot be called keyboard_callback() due to name clash */
void keyboard_callback_rpcemu(void)
{
 ...
 */
uint8_t keyboard_status_read(void)
{
    return kbd.stat;
}
 ...
 * @param v Value to write
 */
void mouse_control_write(uint8_t v)
{
 ...
 */
void mouse_data_write(uint8_t val)
{
 ...
 * @return Value of register
 */
uint8_t mouse_status_read(void)
{
    return msstat;
}
 ...
 */
static void mouse_send(uint8_t v)
{
    msdata = v;
...
static void mouse_process(void)
{
    ...
    if (x == 0 && y == 0 && (mouseb == oldmouseb) && (mouse_type == 0 || (mouse_type != 0 && z == oldz)))
    {
        return;
    }
 ...
 * @param y
 */
void mouse_mouse_move(int x, int y)
{
    assert(mousehack);
 ...
 * @param dy
 */
void mouse_mouse_move_relative(int dx, int dy)
{
    assert(!mousehack);
 ...
 */
void mouse_mouse_press(int buttons)
{
 ...
 * @param buttons Mouse button bitfield
 */
void mouse_mouse_release(int buttons)
{
    mouse.buttons &= ~buttons;
 ...
 */
void keyboard_key_press(const uint8_t *scan_codes)
{
    if (scan_codes == NULL)
    {
        return;
 ...
 * @param scan_codes pointer to array of ps2 set2 scan codes
 */
void keyboard_key_release(const uint8_t *scan_codes)
{
    if (scan_codes == NULL)
    {
        return;
 ...
 */
static void mouse_osunits_to_host(int osx, int osy, int *x, int *y)
{
    int32_t xeig = 1;
    int32_t yeig = 1;
    int double_x;
 ...
 */
static void mouse_get_osxy(int *x, int *y, int *osx, int *osy)
{
    int32_t xeig = 1;
    int32_t yeig = 1;
    int double_x;
    ...
    *y = (((vidc_get_ysize() - 1) << yeig) - *osy) >> yeig;
    ...
    if ((vidc_x != *x) || (vidc_y != *y))
    {
        ...
        if ((mouse_hack.boundbox.left >= 0) || (mouse_hack.boundbox.right < screen_osx) || (mouse_hack.boundbox.bottom >= 0) || (mouse_hack.boundbox.top < screen_osy))
        {
 ...
 */
void mouse_hack_osword_21_4(uint32_t a)
{
    int x;
    int y;
    int osx;
 ...
 */
void mouse_hack_get_pos(int *x, int *y)
{
    int osx;
    int osy;
 ...
 */
void mouse_hack_osword_21_0(uint32_t a)
{
 ...
 */
void mouse_hack_osbyte_106(uint32_t a)
{
    assert(mousehack);
 ...
 * @param a Address of OS_Word 21 parameter block
 */
void mouse_hack_osword_21_1(uint32_t a)
{
 ...
 * @param a Address of OS_Word 21 parameter block
 */
void mouse_hack_osword_21_3(uint32_t a)
{
    int16_t osx;
    int16_t osy;
    int x;
