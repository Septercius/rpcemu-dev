...
static uint32_t shift5(uint32_t opcode, uint32_t shiftmode, uint32_t shiftamount, uint32_t rm)
{
    switch (shiftmode)
    {
        case 0: /* LSL */
...
void exception(uint32_t mmode, uint32_t address, uint32_t diff)
{
    uint32_t link;
 ...
 */
static void arm_unpredictable(uint32_t opcode)
{
