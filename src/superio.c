...
static void superio_smi_update(void)
{
...
void superio_smi_setint1(uint8_t i)
{
    gp_regs[0x0e] |= i;
    superio_smi_update();
}

void superio_smi_setint2(uint8_t i)
{
    gp_regs[0x0f] |= i;
    superio_smi_update();
}

void superio_smi_clrint1(uint8_t i)
{
    gp_regs[0x0f] &= ~i;
    superio_smi_update();
}

void superio_smi_clrint2(uint8_t i)
{
    gp_regs[0x0f] &= ~i;
    superio_smi_update();
}
 ...
 */
static void superio_config_reg_write(uint8_t configreg, uint8_t val)
{
    ...
    switch (configreg)
    {
        case 1:
            ...
            if ((val & 0x3) != 0x3)
            {
                UNIMPLEMENTED("SuperIO Conf Reg", "Unsupported Parallel Port address 0x%x", val & 0x3);
            }
            break;
        ...
        case 2:
            ...
            if ((val & 0x3) != 0x0)
            {
                UNIMPLEMENTED("SuperIO Conf Reg", "Unsupported Serial Port 1 address 0x%x", val & 0x3);
            }
            if (((val >> 4) & 0x3) != 0x1)
            {
                UNIMPLEMENTED("SuperIO Conf Reg", "Unsupported Serial Port 2 address 0x%x", (val >> 4) & 0x3);
            }
            break;
        ...
        case 5:
            ...
            if ((val & 0x1) != 0x0)
            {
                UNIMPLEMENTED("SuperIO Conf Reg", "Unsupported FDC address 0x%x", val & 0x1);
            }
            /* Don't reconfigure IDE address */
            if ((val & 0x2) != 0x0)
            {
                UNIMPLEMENTED("SuperIO Conf Reg", "Unsupported IDE address 0x%x", val & 0x2);
            }
            break;
    }
...
void superio_write(uint32_t addr, uint32_t val)
{
    ...
    else if ((addr >= 0x278) && (addr <= 0x27f))
    {
        ...
        {
            UNIMPLEMENTED("Parallel write", "Unknown register 0x%03x", addr);
        }
    }
    ...
    else if ((addr >= 0x3f8) && (addr <= 0x3ff))
    {
        ...
        {
            UNIMPLEMENTED("Serial1 write", "Unknown register 0x%03x", addr);
        }
    }
    else
    {
        UNIMPLEMENTED("SuperIO write", "Unknown register 0x%03x", addr);
    }
}
...
uint8_t superio_read(uint32_t addr)
{
    ...
    else if ((addr >= 0x278) && (addr <= 0x27f))
    {
        ...
        {
            UNIMPLEMENTED("Parallel read", "Unknown register 0x%03x", addr);
        }
    }
    else if ((addr >= 0x3f0) && (addr <= 0x3f7))
    {
    ...
    else if ((addr >= 0x3f8) && (addr <= 0x3ff))
    {
        ...
        {
            UNIMPLEMENTED("Serial1 read", "Unknown register 0x%03x", addr);
        }
    }
    else
    {
        UNIMPLEMENTED("SuperIO read", "Unknown register 0x%03x", addr);
    }
