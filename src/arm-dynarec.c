...
static inline int arm_opcode_needs_pc(uint32_t opcode)
{
...
static inline int arm_opcode_may_abort(uint32_t opcode)
{
 ...
 */
static inline OpFn arm_opcode_fn(uint32_t opcode)
{
    if (arm.arch_v4)
    {
    ...
    for (linecyc = 256; linecyc >= 0; linecyc--)
    {
        if (!isblockvalid(PC))
        {
            ...
            do
            {
                ...
                {
                    blockend = 1;
                } /* Always end block on branches */
                if ((opcode & 0x0c000000) == 0x0c000000)
                {
                    blockend = 1;
                } /* And SWIs and copro stuff */
                if (!(opcode & 0x0c000000) && (RD == 15))
                {
                    blockend = 1;
                } /* End if R15 can be modified */
                if ((opcode & 0x0e108000) == 0x08108000)
                {
                    blockend = 1;
                } /* End if R15 reloaded from LDM */
                if ((opcode & 0x0c100000) == 0x04100000 && (RD == 15))
                {
                    blockend = 1;
                } /* End if R15 reloaded from LDR */
