 ...
 * settings.
 */
static void cmos_update_settings(void)
{
    ...
    if (tloc->tm_isdst >= 0)
    {
        ...
        { /* Is not in Daylight Savings Time */
            cmosram[0x2c] &= ~(1u << 7);
        }
    }
...
void cmos_init(void)
{
    ...
    if (cmosf)
    {
        ...
        {
            fatal("Unable to read from CMOS file '%s', %s", fn, strerror(errno));
        }
        fclose(cmosf);
    }
    else
...
void savecmos(void)
{
    ...
    if (cmosf)
    {
        ...
        {
            fatal("Unable to write CMOS file '%s': %s", fn, strerror(errno));
 ...
 */
static int pcf8583_write(void *dev, uint8_t data)
{
    PCF8583 *pcf = dev;
 ...
 * @return I2C state-machine return code
 */
static int pcf8583_read(void *dev, uint8_t *data)
{
    PCF8583 *pcf = dev;
 ...
 * @return I2C state machine return code
 */
static int pcf8583_start(void *dev, int i2c_addr, int operation)
{
    PCF8583 *pcf = dev;
 ...
 * @param dev Pointer to the PCF8583 data block
 */
static void pcf8583_stop(void *dev)
{
    PCF8583 *pcf = dev;
...
static const I2C_SlaveOps pcf8583_ops = { pcf8583_start, pcf8583_stop, pcf8583_write, pcf8583_read, NULL };
...
static uint8_t spd_data[128] = { 128, 8, 4, 12, 10, 1, 64, 0, 0, 1, 1, 0, 0, 8, 0, 0, 0xf, 2, 0x7f, 0x7f, 0x7f, 0x00, 0x3f, 0x10, 0x10, 0x10, 0x10, 1, 1, 1, 1, 0x20 };
 ...
 * @return I2C state-machine return code
 */
static int spd_write(void *dev, uint8_t data)
{
    SPD *spd = dev;
 ...
 * @return I2C state-machine return code
 */
static int spd_read(void *dev, uint8_t *data)
{
    SPD *spd = dev;
 ...
 * @return I2C state machine return code
 */
static int spd_start(void *dev, int i2c_addr, int operation)
{
    NOT_USED(dev);
    NOT_USED(i2c_addr);
    NOT_USED(operation);
...
static const I2C_SlaveOps spd_ops = { spd_start, spd_stop, spd_write, spd_read, NULL };
 ...
 */
static void reset_serdes(I2C_SerDes *serdes)
{
 ...
 */
void cmosi2cchange(int scl, int sda)
{
